...
export async function fetchStudentDocumentsData(
  ...
  return (data ?? []).map((d) => ({
    ...
    status: (d.status as import("@/lib/student-documents").DocumentStatus) || "awaiting_verification",
    rejectionReason: d.rejection_reason ?? null,
    verifiedAt: d.verified_at ?? null,
    verifiedByCounsellorId: d.verified_by_counsellor_id ?? null,
    verifiedByCounsellorName: d.verified_by_counsellor_name ?? null,
